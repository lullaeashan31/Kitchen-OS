...
@section('content')
    <div class="flex gap-4" style="flex-wrap: wrap; margin-bottom: 2rem;">
        @if(auth()->user()->isStaff())
            ...
            @if($todaysProduction)
                <a href="{{ route('production.show', $todaysProduction) }}"
                    ...
                    <div class="card"
                        style="background: linear-gradient(135deg, var(--primary-color), var(--primary-hover)); color: white; transition: transform 0.2s;">
            ...
            @else
                <div class="card flex-1 min-w-[300px]" style="background: #f3f4f6; color: var(--text-muted);">
                    <h3 style="margin: 0;">No Production Today</h3>
                </div>
            @endif

            <div class="card flex-1 min-w-[300px]" style="border-left: 4px solid var(--warning-color);">
                ...
                @else
                    <ul style="list-style: none; margin-top: 0.5rem;">
                        @foreach($myTasks->take(3) as $task)
                            <li style="border-bottom: 1px solid #eee; padding: 0.5rem 0; font-size: 0.9rem;">
                                {{ $task->title }} <span
        ...
        @else
            ...
            <div class="card flex-1 min-w-[200px]" style="border-top: 4px solid var(--primary-color);">
                <div class="text-muted text-sm uppercase">Total Recipes</div>
                <div style="font-size: 2rem; font-weight: 700; color: var(--text-main);">{{ $stats['total_recipes'] }}</div>
            </div>
            <div class="card flex-1 min-w-[200px]" style="border-top: 4px solid var(--warning-color);">
                <div class="text-muted text-sm uppercase">Pending Drafts</div>
                <div style="font-size: 2rem; font-weight: 700; color: var(--text-main);">{{ $stats['pending_drafts'] }}</div>
                <div class="text-sm text-muted">My Drafts: {{ $stats['my_drafts'] }}</div>
            </div>
            <div class="card flex-1 min-w-[200px]" style="border-top: 4px solid var(--secondary-color);">
            ...
            <div class="card flex-1 min-w-[200px]" style="border-top: 4px solid #ef4444;">
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                                            <div style="flex: 1; height: 6px; background: #e5e7eb; border-radius: 3px;">
                                                <div
                                                    style="width: {{ $day->progress_percentage }}%; height: 100%; background: var(--primary-color); border-radius: 3px;">
                                                </div>
                                            </div>
